...
description: '为什么 Mooncake 需要一个统一的 Python 源码根、干净的 wheel staging、稳定的公开 API，以及由 Git tag 驱动的发布闭环。'
...
目标方案把所有受版本控制的 Python 代码统一放到 `python/mooncake`，把原生扩展改成私有实现模块，再通过稳定的公开 facade 暴露 API。wheel 只在干净的构建目录中组装，并在安装到隔离环境后再进入发布流程。
...
2. **源码与产物隔离。** `.so`、可执行文件和组装后的 metadata 只进入 `build/`。
3. **公开 API 与实现文件名解耦。** 用户导入稳定的 Python facade，原生模块使用私有名称。
4. **可选能力按需安装。** 核心 import 不加载 vLLM、Torch 或无关硬件 runtime。
5. **测试最终交付物。** 发布门禁安装并运行 wheel，而不是只测试源码树。
6. **发布由 immutable tag 驱动。** tag、`pyproject.toml` 和 wheel metadata 必须一致。
...
新的构建链只有一个汇合点：`build/wheel-stage`。

```mermaid
flowchart LR
    P[python/mooncake<br/>tracked Python source]
    C[CMake build<br/>native artifacts]
    I[CMake install<br/>python-runtime]
    S[build/wheel-stage<br/>clean directory]
    W[wheel]
    V[isolated install<br/>and smoke tests]
    A[auditwheel]
    R[release artifact]
...
CMake 继续负责原生编译，但增加一个相对路径的 `python-runtime` install component：
...
cmake --build build/release --parallel 128

cmake --install build/release \
  --prefix build/wheel-stage/cuda-py312 \
  --component python-runtime
```

随后 staging 脚本把 `python/mooncake` 与 install 结果放到同一个空目录中，再调用 setuptools/build backend。

每次构建都先创建新的 variant/Python staging，不再需要“构建后恢复源码”。CUDA 13、non-CUDA、EFA 等分发名称只修改 staging 内的 metadata。
...
manifest、codec、reshard contract 和 adapter 等工作不应该要求先编译完整 C++：

```bash
python3 -m venv .venv
source .venv/bin/activate

pip install -e './python[dev,structured]'
pytest python/tests/unit
```

这要求 `mooncake.__init__` 足够轻量：不能在包导入时就尝试加载所有 native extension、Torch 或 vLLM。

### 原生联调

需要 binding 时，开发环境使用一个与 wheel 布局相同的 staging tree：
...
cmake --build build/dev --parallel 128

./scripts/stage_python_dev.sh \
  --build-dir build/dev \
  --output-dir build/python-dev

PYTHONPATH="$PWD/build/python-dev" \
  pytest python/tests/integration
```

dev staging 可以 symlink Python 源码，让 `.py` 修改立即生效；原生产物仍来自 CMake build/install。修改 C++ 后重新 build，修改纯 Python 则不需要。
...
`python/pyproject.toml` 是 package version 的 source of truth；Git tag 选择不可变的 release commit。二者必须一致：
...
真正的复杂性应该被 reusable builders 吸收，而不是复制在九个入口 workflow 中。
...
- CUDA、CUDA 13、non-CUDA 使用标准 matrix；
- EFA 保留自己的环境 matrix；
- ROCm、NPU、MUSA 继续拥有独立 builder，因为 runner 和 toolchain 确实不同；
- builder 只构建、验证和上传 Actions artifact；
- publisher 才能发布 PyPI 和更新 GitHub Release。
...
### 第三阶段：替换 in-source staging

- 增加 CMake `python-runtime` install component；
- 实现 clean wheel staging 和 dev staging；
- 将 variant metadata 改为 staging-only；
- 在隔离环境验证每个 wheel；
- 删除复制、恢复和源码内清理逻辑。
...
- 标准变体改为 matrix；
- 加入 tag/version 校验；
- 集中 PyPI 与 GitHub Release publication；
...
| 源码与产物             | 同一目录混合           | source、stage、dist 分离          |
| 本地开发               | 依赖复制和路径补丁     | editable install + dev staging    |
| 原生 API               | `.so` 直接占公开模块名 | private extension + public facade |
| 依赖                   | 隐式或过度安装         | core/extras/hardware 分层         |
| wheel 验证             | 容易被源码路径影响     | 隔离安装验证                      |
| 版本发布               | 多入口重复策略         | tag/version invariant + 单一编排  |
...
- 开发者知道应该编辑哪一份文件；
- CMake 知道应该安装哪些原生产物；
- wheel builder 不会修改源码；
- CI 测试用户真正安装的文件；
- release workflow 对版本和 artifact 有唯一判断；
- 上游项目获得可预测的兼容窗口。
...
统一 `python/` 目录并不是为了追求某种流行仓库外观。它要建立四个稳定边界：

```text
python/mooncake          = 唯一 Python 源码
CMake python-runtime     = 唯一原生产物安装契约
build/wheel-stage        = 唯一 wheel 汇合点
Git tag + pyproject      = 唯一发布版本事实
```

只要这四个边界成立，目录、测试、依赖和 release YAML 的简化都会成为自然结果；如果只移动文件而保留原来的 copy-and-restore 构建方式，复杂度只会换一个位置继续存在。
